...
class plgVmPaymentCmCoupon extends vmPSPlugin {
	...
	public function plgVmgetPaymentCurrency($virtuemart_paymentmethod_id, &$paymentCurrency){
		...
		if(file_exists($cm_file)) {
			require_once $cm_file;
			return CmCouponVirtuemartCouponHandler::process_autocoupon();
		}
...
//echo '<pre>'; print_r($cart_prices); exit;
		...
		if((int)$cmparams->get('virtuemart_inject_totals',0)==1) {
		// get cmcoupon discount since discount not available in virtuemart object at this point
			$coupon_discount = 0;
			$session = JFactory::getSession();
			$coupon_row = $session->get('coupon', '', 'cmcoupon');
			if(!empty($coupon_row)) {
