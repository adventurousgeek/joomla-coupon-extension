...
class mod_cmcoupon_balanceInstallerScript {
	...
	private function fixManifest($adapter) {
		$filesource = $adapter->get('parent')->getPath('source').'/mod_cmcoupon_balance_j3.xml';
		$filedest = $adapter->get('parent')->getPath('extension_root').'/mod_cmcoupon_balance.xml';
		...
		@unlink($adapter->get('parent')->getPath('extension_root').'/mod_cmcoupon_balance_j3.xml');
