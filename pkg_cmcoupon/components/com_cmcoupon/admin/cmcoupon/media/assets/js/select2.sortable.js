 ...
 *
 * @package JQuery Select2 Sortable
 * Author      : Vafour
			...
			select2SortableOrder: function(){
				...
				$this.each(
					function(){
						...
						sorted = $(
							$select2.find( '.select2-choices li[class!="select2-search-field"]' ).map(
								function() {
									if ( ! this) {
										return undefined;
									}
						...
						function(){
							...
							switch (options.bindOrder) {
								case 'sortableStop':
									...
									$select2choices.on(
										"sortstop.select2sortable", function( event, ui ) {
								...
								default:
									...
									$select.closest( 'form' ).unbind( 'submit.select2sortable' ).on(
										'submit.select2sortable', function(){
											$select.select2SortableOrder();
										}
									);
							}
