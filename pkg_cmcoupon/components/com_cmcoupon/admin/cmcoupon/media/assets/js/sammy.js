...
(function(factory){
	...
	$.extend(
		Sammy.DefaultLocationProxy.prototype , {
			...
			bind: function() {
				...
				$( window ).bind(
					'hashchange.' + this.app.eventNamespace(), function(e, non_native) {
				...
				if (_has_history && ! app.disable_push_state) {
					// bind to popstate.
					$( window ).bind(
						'popstate.' + this.app.eventNamespace(), function(e) {
							app.trigger( 'location-changed' );
						}
					);
					// bind to link clicks that have routes.
					$( document ).delegate(
						'a', 'click.history-' + this.app.eventNamespace(), function (e) {
	...
	Sammy.Application.prototype = $.extend(
		{}, Sammy.Object.prototype, {
			...
			mapRoutes: function(route_array) {
				var app = this;
				$.each(
					route_array, function(i, route_args) {
			...
			run: function(start_url) {
				...
				$.each(
					this.listeners.toHash(), function(name, callbacks) {
						$.each(
							callbacks, function(i, listener_callback) {
			...
			unload: function() {
				...
				// unbind all events.
				$.each(
					this.listeners.toHash() , function(name, listeners) {
						$.each(
							listeners, function(i, listener_callback) {
			...
			bindToAllEvents: function(callback) {
				...
				$.each(
					this.APP_EVENTS, function(i, e) {
						app.bind( e, callback );
					}
				);
				// next, bind to listener names (only if they dont exist in APP_EVENTS).
				$.each(
					this.listeners.keys( true ), function(i, name) {
			...
			runRoute: function(verb, path, params, target) {
				...
				if (route) {
					...
					$.each(
						arounds.reverse(), function(i, around) {
	...
	Sammy.RenderContext.prototype = $.extend(
		{}, Sammy.Object.prototype, {
			...
			renderEach: function(location, name, data, callback) {
				...
				return this.load( location ).then(
					function(content) {
						var rctx = this;
						if ( ! data) {
	...
	Sammy.EventContext.prototype = $.extend(
		{}, Sammy.Object.prototype, {
